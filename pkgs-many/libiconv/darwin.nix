...
mkAppleDerivation (finalAttrs: {
  ...
  patches = [
    ...
    ./darwin/patches/0001-Support-static-module-loading.patch
    # Avoid out of bounds write with ISO-2022
    ./darwin/patches/0002-Fix-ISO-2022-out-of-bounds-write-with-encoded-charac.patch
  ];
  ...
  postPatch = ''
    ...
    cp ${./darwin/nixpkgs_test.c} tests/libiconv/nixpkgs_test.c
  ''
  + lib.optionalString hostPlatform.isStatic ''
    cp ${./darwin/static-modules.gperf} static-modules.gperf
  '';
