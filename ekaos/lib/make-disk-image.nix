/*
  Disk image builder for ekaos systems
  ...
  - transfer from the temporary store the additional paths registered to the installed system
...
*/
{
  ...
  # The ekaos configuration to be installed onto the disk image.
  config,
  ...
  # Filesystem label
  label ? if onlyNixStore then "nix-store" else "ekaos",

  # The initial configuration file to be copied to
  # /etc/ekaos/configuration.nix.
  configFile ? null,
  ...
  # Whether a nix channel should be made available inside the image.
  # Note: ekaos typically does not use channels.
  copyChannel ? false,
...
let
  ...
  # Channel sources - not typically used in ekaos
  channelSources = pkgs.runCommand "ekaos-channel" { } ''
    mkdir -p $out
    echo "ekaos does not use channels by default" > $out/README
  '';
  ...
  buildImage = pkgs.vmTools.runInLinuxVM (
    pkgs.runCommand name
      ...
      ''
        ...
        # Install a configuration.nix (or ekaos equivalent)
        mkdir -p /mnt/etc/ekaos
        ${lib.optionalString (configFile != null) ''
          cp ${configFile} /mnt/etc/ekaos/configuration.nix
        ''}

        ${lib.optionalString installBootLoader ''
          # In this throwaway resource, we only have /dev/vda, but the actual VM may refer to another disk for bootloader, e.g. /dev/vdb
          # Use this option to create a symlink from vda to any arbitrary device you want.
          ${lib.optionalString (config.boot.loader.grub.enable or false) (
