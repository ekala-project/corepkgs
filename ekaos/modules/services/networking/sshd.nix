...
let
  ...
  portsList =
    if builtins.isList cfg.settings.ports then cfg.settings.ports else [ cfg.settings.ports ];
  ...
  sshdConfig = pkgs.writeText "sshd_config" ''
    ...
    # Authorized keys
    ${optionalString (
      cfg.settings.authorizedKeysFiles != [ ]
    ) "AuthorizedKeysFile ${concatStringsSep " " cfg.settings.authorizedKeysFiles}"}
    ...
    services.openssh = {
      ...
      settings = mkOption {
        type = types.submodule {
          options = {
            ports = mkOption {
              type = types.either types.port (types.listOf types.port);
              default = 22;
              example = [
                22
                2222
