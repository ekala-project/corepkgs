...
let
  ...
  mkError = message: { type = "error"; inherit message; };
  mkWarning = message: { type = "warning"; inherit message; };
  ...
  hasPlatformOptions = platformName: platformOpts:
    let
      ...
      ignoredKeys = {
        ...
        # rcd defaults
        rcd = [ "variant" "rcProvide" "rcRequire" "rcKeywords" ];  # All have non-trivial defaults
        # runit defaults
        runit = [ "superviseDirectory" ];  # Has default "/etc/sv/${name}"
        # launchd defaults
        launchd = [ "runAtLoad" "keepAlive" "processType" "sessionCreate" "enableTransactions" "abandonProcessGroup" "label" ];
      };
  ...
  checkPlatformSpecificOptions = builder: config:
    let
      ...
      platforms = [ "systemd" "launchd" "runit" "rcd" ];
    in
  ...
  checkUnsupportedCommonOptions = builder: config:
    let
      warnings = []
        ...
        ++ optional (config.user != "root" && builder == "rcd" && (config.rcd.variant or "freebsd") != "openbsd")
          (mkWarning "User switching on ${builder} (${config.rcd.variant or "freebsd"}) may require manual setup with daemon(8) or su(1)")
  ...
  checkConfigConflicts = config:
    let
      errors = []
        ...
        ++ optional (!(builtins.elem config.restartPolicy [ "always" "on-failure" "on-abnormal" "on-abort" "on-watchdog" "never" ]))
  ...
  validateService = builder: name: config:
    let
  ...
  formatValidationResult = result:
    let
      ...
      errorSection = if result.errors != [] then
        "ERRORS (build will fail):\n${concatStringsSep "\n" errorLines}\n"
      else "";

      warningSection = if result.warnings != [] then
  ...
  validateServices = builder: services:
    let
