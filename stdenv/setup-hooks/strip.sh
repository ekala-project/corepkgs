...
_doStrip() {
    ...
    # Provide defaults for strip directory lists.
    # Use array syntax; ${var[*]} flattens for stripDirs which uses word-splitting.
    if [ -z "${stripDebugList+set}" ]; then
        stripDebugList=(lib lib32 lib64 libexec bin sbin Applications Library/Frameworks)
    fi
