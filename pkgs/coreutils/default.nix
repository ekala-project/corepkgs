{
  ...
  coreutils,
  gmpSupport ? true,
  aclSupport ? lib.meta.availableOn mkEkaPackage.stdenv.hostPlatform (
    mkEkaPackage.scopes.hostTarget.acl or { meta = { }; }
  ),
  attrSupport ? lib.meta.availableOn mkEkaPackage.stdenv.hostPlatform (
    mkEkaPackage.scopes.hostTarget.attr or { meta = { }; }
  ),
...
mkEkaPackage rec {
  ...
  commands =
    ...
    // optionalAttrs stdenv.hostPlatform.isCygwin {
      # due to patch
  ...
  libraries =
    scope:
    { }
    // optionalAttrs aclSupport { inherit (scope) acl; }
    // optionalAttrs attrSupport { inherit (scope) attr; }
    // optionalAttrs gmpSupport { inherit (scope) gmp; }
    // optionalAttrs withOpenssl { inherit (scope) openssl; }
    // optionalAttrs selinuxSupport { inherit (scope) libselinux libsepol; }
    # TODO(@Ericson2314): Investigate whether Darwin could benefit too
    // optionalAttrs (isCross && stdenv.hostPlatform.libc != "glibc") { inherit (scope) libiconv; };
  ...
  # (!isCross) && (stdenv.hostPlatform.libc == "glibc" || stdenv.hostPlatform.libc == "musl") && !stdenv.hostPlatform.isAarch32;
  ...
  passthru = {
    tests = {
      ...
      simple = runCommand "coreutils-test" { } ''
        ${coreutils}/bin/echo "hello" | ${coreutils}/bin/tr a-z A-Z > $out
        ${coreutils}/bin/test "$(${coreutils}/bin/cat $out)" = "HELLO"
      '';
    };
  }
