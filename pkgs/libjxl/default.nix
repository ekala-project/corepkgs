...
stdenv.mkDerivation rec {
  ...
  cmakeEntries = {
    ...
    JPEGXL_FORCE_SYSTEM_BROTLI = true;

    # Use our version of gtest
    JPEGXL_FORCE_SYSTEM_GTEST = true;

    JPEGXL_ENABLE_SKCMS = false;
    JPEGXL_FORCE_SYSTEM_LCMS2 = true;
    ...
    JPEGXL_ENABLE_TCMALLOC = false;

    # Enable plugins, such as:
    # * the `gdk-pixbuf` one, which allows applications like `eog` to load jpeg-xl files
    ${if enablePlugins then "JPEGXL_ENABLE_PLUGINS" else null} = true;

    ${if stdenv.hostPlatform.isStatic then "JPEGXL_STATIC" else null} = true;

    ${if stdenv.hostPlatform.isAarch32 then "JPEGXL_FORCE_NEON" else null} = true;
  };
