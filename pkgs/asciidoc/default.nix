  ...
  + (
    if enableStandardFeatures then
      ''
        ...
        sed -e "s|^ENV =.*|ENV = dict(XML_CATALOG_FILES='${docbook-xml-dtd.v4_5}/xml/dtd/docbook/catalog.xml ${docbook-xsl-ns}/xml/xsl/docbook/catalog.xml ${docbook-xsl-nons}/xml/xsl/docbook/catalog.xml', PATH='${
          lib.makeBinPath [
            texlive
            coreutils
            gnused
      ...
      ''
        sed -e "s|^ENV =.*|ENV = dict(XML_CATALOG_FILES='${docbook-xml-dtd.v4_5}/xml/dtd/docbook/catalog.xml ${docbook-xsl-ns}/xml/xsl/docbook/catalog.xml ${docbook-xsl-nons}/xml/xsl/docbook/catalog.xml', **(dict(filter(lambda v: v[0] == 'SOURCE_DATE_EPOCH', os.environ.items()))))|" \
            -e "s|^XSLTPROC =.*|XSLTPROC = '${libxslt.bin}/bin/xsltproc'|" \
            -e "s|^XMLLINT =.*|XMLLINT = '${libxml2.bin}/bin/xmllint'|" \
            -i asciidoc/a2x.py
      ''
